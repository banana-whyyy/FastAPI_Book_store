...
from app.schemas import AuthorCreate, AuthorResponse, AuthorWithBookResponse
...
async def add_authors(author: AuthorCreate, db: AsyncSession=Depends(get_db), _: User=Depends(get_admin_user)):
...
@router.get("/{author_id}", response_model=AuthorWithBookResponse)
async def get_author_detail(author_id: int, db: AsyncSession=Depends(get_db)):
