...
from app.models import Book, Author, User, Order
from app.schemas import BookCreate, AuthorCreate, UserCreate, BookUpdate, OrderCreate
...
async def update_book(db: AsyncSession, book_update: BookUpdate, book_id: int) -> Book:
...
async def get_user(db: AsyncSession, user_id: int) -> User | None:
    result = await db.execute(
        select(User).options(selectinload(User)).where(User.id == user_id)
    )
    return result.scalar_one_or_none()
...
async def get_user_by_email(db: AsyncSession, user_email: str) -> User | None:
    result = await db.execute(
        select(User).options(selectinload(User)).where(User.email == user_email)
    )
    return result.scalar_one_or_none()
